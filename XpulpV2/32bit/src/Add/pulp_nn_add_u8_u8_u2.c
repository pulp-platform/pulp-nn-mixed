    ...
    int      out_requant_flag)
{
    ...
    int ch_im_in1 = ch_im_in << 0;
    int ch_im_in2 = ch_im_in << 0;
    ...
    for (int i=start; i<((stop * ch_im_out_r * dim_im_in_x) >> 0); i++)
    {
        target1_ext = target1;
        target1+=4;
        ...
        out1 = bitins(out1, 0xf3, out2, 0x0c, 2);
        out1 = bitins(out1, 0xcf, out3, 0x30, 4);
        *pOutBuffer = bitins(out1, 0x3f, out4, 0xc0, 6);
        pOutBuffer++;
    }
