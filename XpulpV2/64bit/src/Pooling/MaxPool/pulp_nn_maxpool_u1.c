...
void __attribute__ ((noinline)) pulp_nn_maxpool_u1(
  uint8_t * pIn,
  uint8_t * pOut,
  ...
  uint16_t  stride_y)
{
  ...
  for (i_y = start; i_y < stop; i_y++)
  {
    for (i_x = 0; i_x < dim_im_out_x; i_x++)
    {
      ...
      {
        pulp_nn_compare_and_replace_if_larger_u1(target, win_start, ch_im_in_r);
      }
    }
  }
  ...
  for (i_y = start2; i_y < stop2; i_y++)
  {
    ...
    {
      pulp_nn_compare_and_replace_if_larger_u1(target, row_start, dim_im_out_x * ch_im_in_r);
    }
  }
  pi_cl_team_barrier(0);
}
