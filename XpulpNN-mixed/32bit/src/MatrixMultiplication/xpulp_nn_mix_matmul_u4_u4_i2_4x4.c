/*
 * xpulp_nn_mix_matmul_u4_u4_i2_4x4.c
...
uint8_t * __attribute__((noinline)) xpulp_nn_mix_matmul_u4_u4_i2_4x4(
                        uint8_t *pIn,
                        int8_t *pBias,
                        uint8_t *pOut,
                        uint8_t *pOut2,
                        ...
                        uint8_t flag_batch_norm)
{
  ...
  for(int i=0; i < (ch_out >> 2); i++)
  {
    ...
    for(int j=0; j<(num_col_im2col >> 4); j++)
    {
      ...
      sum13 = MacLoad8(0, 0, 0, 1, ptrA, sum13);
      sum14 = MacLoad8(0, 0, 1, 1, ptrA, sum14);
      sum15 = MacLoad8(0, 0, 2, 1, ptrA, sum15);
      sum16 = MacLoad8(0, 1, 3, 1, ptrB, sum16);
      ptrB  = MacLoadUpdate(ptrB);
    ...
    if(col_cnt_im2col)
    {
      ...
      do
      {
        ...
        uint8_t inB3 = (uint8_t)bitextu((uint32_t) *pB3, 4, 0);
        uint8_t inB4 = (uint8_t)bitextu((uint32_t) *pB4, 4, 0);
        ...
        inB3 = (uint8_t)bitextu((uint32_t) *pB3, 4, 4);
        inB4 = (uint8_t)bitextu((uint32_t) *pB4, 4, 4);
        ...
        inB3 = (uint8_t)bitextu((uint32_t) *pB3, 4, 0);
        inB4 = (uint8_t)bitextu((uint32_t) *pB4, 4, 0);
  ...
  while(chan_left)
  {
    ...
    for(int j=0; j < (num_col_im2col >> 4); j++)
    {
      ...
      sum2 = MacLoad8(0, 1, 0, 1, ptrB, sum2);
      ptrB = MacLoadUpdate(ptrB);

      sum3 = MacLoad8(0, 1, 0, 0, ptrB, sum3);
      ptrB = MacLoadUpdate(ptrB);

      sum4 = MacLoad8(0, 1, 0, 1, ptrB, sum4);
      ...
      sum4 = MacLoad8(1, 0, 0, 1, ptrA, sum4);
      ptrA = MacLoadUpdate(ptrA);
    }
    asm volatile ("csrr %0, 0x101" : "=r" (pA));
    pA-=4;
