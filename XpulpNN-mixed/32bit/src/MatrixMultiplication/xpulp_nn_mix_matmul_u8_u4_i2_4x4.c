/*
 * xpulp_nn_mix_matmul_u8_u4_i2_4x4.c
...
uint8_t * __attribute__((noinline)) xpulp_nn_mix_matmul_u8_u4_i2_4x4(
                        uint8_t *pIn,
                        int8_t *pBias,
                        uint8_t *pOut,
                        uint8_t *pOut2,
                        ...
                        uint8_t flag_batch_norm)
{
  ...
  for(int i=0; i < (ch_out >> 2); i++)
  {
    ...
    for(int j=0; j<(num_col_im2col >> 4); j++)
    {
      ...
      sum6 = MacLoad4(0, 0, 1, 1, ptrA, sum6);
      sum7 = MacLoad4(0, 0, 2, 1, ptrA, sum7);
      sum8 = MacLoad4(0, 1, 3, 1, ptrB, sum8);
      ptrB = MacLoadUpdate(ptrB);

      sum9  = MacLoad4(0, 0, 0, 0, ptrA, sum9);
      sum10 = MacLoad4(0, 0, 1, 0, ptrA, sum10);
      sum11 = MacLoad4(0, 0, 2, 0, ptrA, sum11);
      sum12 = MacLoad4(0, 1, 3, 0, ptrB, sum12);
      ptrB  = MacLoadUpdate(ptrB);

      sum13 = MacLoad4(0, 0, 0, 1, ptrA, sum13);
      sum14 = MacLoad4(0, 0, 1, 1, ptrA, sum14);
      sum15 = MacLoad4(0, 0, 2, 1, ptrA, sum15);
      sum16 = MacLoad4(0, 1, 3, 1, ptrB, sum16);
      ptrB  = MacLoadUpdate(ptrB);
      ...
      sum14 = MacLoad4(1, 0, 1, 1, ptrA, sum14);
      ptrA  = MacLoadUpdate(ptrA);

      sum15 = MacLoad4(1, 0, 2, 1, ptrA, sum15);
      ptrA  = MacLoadUpdate(ptrA);

      sum16 = MacLoad4(1, 0, 3, 1, ptrA, sum16);
      ptrA  = MacLoadUpdate(ptrA);
