/*
 * xpulp_nn_mix_depthwise_u2_u8_i2.c
...
void xpulp_nn_mix_depthwise_u2_u8_i2(
                        uint8_t *pIn,
      ...
      do
      {
        ...
        if(dim_out_y & 0x1){
          ...
          if (flag_batch_norm && flag_relu)
          {
            ...
            *(pOutBuffer2 + 1) = pulp_nn_bn_quant_u8(sum2, *(k1 + 1), *(lambda1 + 1), out_shift);
            *(pOutBuffer2 + 2) = pulp_nn_bn_quant_u8(sum3, *(k1 + 2), *(lambda1 + 2), out_shift);
            *(pOutBuffer2 + 3) = pulp_nn_bn_quant_u8(sum4, *(k1 + 3), *(lambda1 + 3), out_shift);
          }
          else
          {
            if(flag_relu == 1)
            {
              ...
              *(pOutBuffer2 + 1) = pulp_nn_quant_u8(sum2, out_mult, out_shift);
              *(pOutBuffer2 + 2) = pulp_nn_quant_u8(sum3, out_mult, out_shift);
              *(pOutBuffer2 + 3) = pulp_nn_quant_u8(sum4, out_mult, out_shift);
            }
            else
            {
              *pOutBuffer2 = (uint8_t) clip8(sum >> out_shift);
              *(pOutBuffer2 + 1) = (uint8_t) clip8(sum2 >> out_shift);
              *(pOutBuffer2 + 2) = (uint8_t) clip8(sum3 >> out_shift);
              *(pOutBuffer2 + 3) = (uint8_t) clip8(sum4 >> out_shift);
            }
          }
          A_ROLLBACK(a_rollback);
          W_ROLLBACK(w_rollback);
