/*
 * xpulp_nn_mix_depthwise_u8_i8_i8.c
...
void xpulp_nn_mix_depthwise_u8_i8_i8(
                        uint8_t *pIn,
        ...
        do
        {
          ...
          if(leftCnt)
          {
            ...
            do
            {
              ...
              uint8_t x2 = *(uint8_t *) (pIm2Col - 1 + ((dim_out_y >> 1) * stride_y * dim_kernel_x));
              sum += x * w;
              sum2 += x2 * w;
              j++;
            }while(j<leftCnt);
          ...
          if (flag_batch_norm && flag_relu)
          {
            ...
            *pOutBuffer2 = pulp_nn_bn_quant_i8(sum2, *k1, *lambda1, out_shift);
          }
          else
          {
            if(flag_relu == 1)
            {
              ...
              *pOutBuffer2 = pulp_nn_quant_i8(sum2, out_mult, out_shift);
            }
            else
            {
              *pOutBuffer = (int8_t) clips8(sum >> out_shift);
              *pOutBuffer2 = (int8_t) clips8(sum2 >> out_shift);
            }
          }
