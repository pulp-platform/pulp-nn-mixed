/*
 * xpulp_nn_mix_conv_i2_i2_i2_4x4.c
...
void __attribute__((noinline)) xpulp_nn_mix_conv_i2_i2_i2_4x4(
                        int8_t *pIn,
                        ...
                        uint8_t flag_batch_norm)
{
  ...
  for (i_out_y = start_pixel; i_out_y < stop_pixel; i_out_y++)
  {
    for(i_out_x=(section * dim_out_x_r); i_out_x<(dim_out_x_r + (section * (dim_out_x_r + flag_dim_out_x_odd))); i_out_x++)
    {
      ...
      {
        pOutBuffer = xpulp_nn_mix_matmul_i2_i2_i2_4x4(
          pIm2ColBase,
          pBias,
          pOutBuffer,
          pOutBuffer + ch_out_r,
    ...
    if(pIm2Col != pIm2ColBase)
    {
      ...
      for(i = 0; i < ch_out; i++)
      {
        ...
        {
          sum = SumDotps16(*(int8_t *)ptrB, *(int32_t *)ptrA, sum);
          ptrA++;
          ptrB++;
        }
