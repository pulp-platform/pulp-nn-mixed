/*
 * xpulp_nn_mix_linear_u2_u2_i2.c
...
void __attribute__((noinline)) xpulp_nn_mix_linear_u2_u2_i2(
                        uint8_t *pIn,
                        int8_t *pBias,
                        uint8_t *pOut,
                        int8_t *pWeight,
                        ...
                        uint8_t flag_batch_norm)
{
  ...
  for(i=start; i<stop; i+=4)
  {
    ...
    for(int j=0; j < (dim_vec >> 4); j++)
    {
      ...
      sum2 = MacLoad16(1, 0, 1, 0, ptrA, sum2);
      ptrA = MacLoadUpdate(ptrA);
      sum3 = MacLoad16(1, 0, 2, 0, ptrA, sum3);
      ptrA = MacLoadUpdate(ptrA);
      sum4 = MacLoad16(1, 0, 3, 0, ptrA, sum4);
      ptrA = MacLoadUpdate(ptrA);
