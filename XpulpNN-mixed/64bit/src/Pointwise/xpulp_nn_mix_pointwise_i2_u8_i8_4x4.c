/*
 * xpulp_nn_mix_pointwise_i2_u8_i8_4x4.c
...
void __attribute__((noinline)) xpulp_nn_mix_pointwise_i2_u8_i8_4x4(
                        int8_t *pIn,
                        ...
                        uint8_t flag_batch_norm)
{
  ...
  int8_t *pIm2Col = pIm2ColBuffer + (4 * core_id * PACK_INT8_SIZE(ch_in));
  ...
  for (i_out_y = start_pixel; i_out_y < stop_pixel; i_out_y++)
  {
    ...
    {
      xpulp_nn_im2col_i2_to_i8(pIn + (i_out_x * ch_in_r) + (i_out_y * dim_in_x * ch_in_r), pIm2Col, ch_in<<2);
      pOutBuffer = xpulp_nn_mix_matmul_i2_u8_i8_4x4(
          pIm2Col,
          pBias,
          pOutBuffer,
          pOutBuffer + ch_out_r,
