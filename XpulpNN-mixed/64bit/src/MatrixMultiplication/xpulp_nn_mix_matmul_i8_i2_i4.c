/*
 * xpulp_nn_mix_matmul_i8_i2_i4.c
...
uint8_t * __attribute__((noinline)) xpulp_nn_mix_matmul_i8_i2_i4(
                        int8_t *pIn,
                        int8_t *pBias,
                        int8_t *pOut,
                        int8_t *pOut2,
                        ...
                        uint8_t flag_batch_norm)
{
  ...
  for(int i=0; i < (ch_out >> 2); i++)
  {
    ...
    for(int j=0; j<(num_col_im2col >> 3); j++)
    {
      ...
      sum6 = MacLoads4(1, 0, 1, 1, ptrA, sum6);
      ptrA = MacLoadUpdate(ptrA);

      sum7 = MacLoads4(1, 0, 2, 1, ptrA, sum7);
      ptrA = MacLoadUpdate(ptrA);
