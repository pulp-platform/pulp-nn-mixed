/*
 * xpulp_nn_mix_matmul_i2_u8_i2.c
...
uint8_t * __attribute__((noinline)) xpulp_nn_mix_matmul_i2_u8_i2(
                        int8_t *pIn,
                        int8_t *pBias,
                        uint8_t *pOut,
                        uint8_t *pOut2,
                        ...
                        uint8_t flag_batch_norm)
{
  ...
  for(int i=0; i < (ch_out >> 2); i++)
  {
    ...
    for(int j=0; j<(num_col_im2col >> 4); j++)
    {
      ...
      sum6 = MacLoads16(1, 0, 1, 1, ptrA, sum6);
      ptrA = MacLoadUpdate(ptrA);

      sum7 = MacLoads16(1, 0, 2, 1, ptrA, sum7);
      ptrA = MacLoadUpdate(ptrA);

      sum8 = MacLoads16(1, 0, 3, 1, ptrA, sum8);
      ptrA = MacLoadUpdate(ptrA);
