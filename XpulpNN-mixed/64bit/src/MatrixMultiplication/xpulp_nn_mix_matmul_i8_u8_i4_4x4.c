/*
 * xpulp_nn_mix_matmul_i8_u8_i4_4x4.c
...
uint8_t * __attribute__((noinline)) xpulp_nn_mix_matmul_i8_u8_i4_4x4(
                        int8_t *pIn,
                        int8_t *pBias,
                        uint8_t *pOut,
                        uint8_t *pOut2,
                        ...
                        uint8_t flag_batch_norm)
{
  ...
  for(int i=0; i < (ch_out >> 2); i++)
  {
    ...
    for(int j=0; j<(num_col_im2col >> 3); j++)
    {
      ...
      sum13 = MacLoads4(0, 0, 0, 1, ptrA, sum13);
      sum14 = MacLoads4(0, 0, 1, 1, ptrA, sum14);
      sum15 = MacLoads4(0, 0, 2, 1, ptrA, sum15);
      sum16 = MacLoads4(0, 1, 3, 1, ptrB, sum16);
      ptrB  = MacLoadUpdate(ptrB);
  ...
  while(chan_left)
  {
    ...
    for(int j=0; j < (num_col_im2col >> 3); j++)
    {
      ...
      sum4 = MacLoads4(0, 1, 0, 1, ptrB, sum4);
