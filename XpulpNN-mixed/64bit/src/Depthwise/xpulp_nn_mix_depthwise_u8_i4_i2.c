/*
 * xpulp_nn_mix_depthwise_u8_i4_i2.c
...
void xpulp_nn_mix_depthwise_u8_i4_i2(
                        uint8_t *pIn,
        ...
        do
        {
          ...
          if (flag_batch_norm && flag_relu)
          {
            ...
            *(pOutBuffer + 1) = bitins(sum3, n_mask, sum4, mask, off);
            sum5 = pulp_nn_bn_quant_i4(sum5, *k1, *lambda1, out_shift);
            sum6 = pulp_nn_bn_quant_i4(sum6, *(k1 + 1), *(lambda1 + 1), out_shift);
            sum7 = pulp_nn_bn_quant_i4(sum7, *(k1 + 2), *(lambda1 + 2), out_shift);
            sum8 = pulp_nn_bn_quant_i4(sum8, *(k1 + 3), *(lambda1 + 3), out_shift);
        ...
        else
        {
          if(flag_relu == 1)
          {
            ...
            *(pOutBuffer + 1) = bitins(sum3, n_mask, sum4, mask, off);
            ...
            sum6 = pulp_nn_quant_i4(sum6, out_mult, out_shift);
            *pOutBuffer2 = bitins(sum5, n_mask, sum6, mask, off);
            sum7 = pulp_nn_quant_i4(sum6, out_mult, out_shift);
            sum8 = pulp_nn_quant_i4(sum8, out_mult, out_shift);
            *(pOutBuffer2 + 1) = bitins(sum7, n_mask, sum8, mask, off);
          }
          else
