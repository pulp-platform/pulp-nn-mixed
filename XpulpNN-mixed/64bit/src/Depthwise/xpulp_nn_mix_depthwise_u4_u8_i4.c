/*
 * xpulp_nn_mix_depthwise_u4_u8_i4.c
...
void xpulp_nn_mix_depthwise_u4_u8_i4(
                        uint8_t *pIn,
                        ...
                        uint8_t flag_batch_norm)
{
  ...
  int32_t a_rollback = ((dim_out_y >> 1) * stride_y * dim_kernel_x) - a_stride;
  int32_t a_rollback2 = 4 - ((dim_out_y >> 1) * stride_y * dim_kernel_x) - a_stride;
      ...
      do
      {
        ...
        do
        {
          ...
          do
          {
            ...
            ptrB = MacLoadUpdate(ptrB);
            sum3 = MacLoad4(1, 0, 0, 0, ptrA, sum3);
            ptrA = MacLoadUpdate(ptrA);
            sum4 = MacLoad4(1, 0, 1, 1, ptrA, sum4);
            ptrA = MacLoadUpdate(ptrA);
            ptrB = MacLoadInit(0, 1, 0, 0, ptrB);
            A_ROLLBACK(a_rollback);
            j++;
          ...
          if (flag_batch_norm && flag_relu)
          {
            ...
            *pOutBuffer2 = pulp_nn_bn_quant_u8(sum3, *k1, *lambda1, out_shift);
            *(pOutBuffer2 + 1) = pulp_nn_bn_quant_u8(sum4, *(k1 + 1), *(lambda1 + 1), out_shift);
          }
          else
          {
            if(flag_relu == 1)
            {
              ...
              *pOutBuffer2 = pulp_nn_quant_u8(sum3, out_mult, out_shift);
              *(pOutBuffer2 + 1) = pulp_nn_quant_u8(sum4, out_mult, out_shift);
            }
            else
            {
              *pOutBuffer = (uint8_t) clip8(sum >> out_shift);
              *(pOutBuffer + 1) = (uint8_t) clip8(sum2 >> out_shift);
              *pOutBuffer2 = (uint8_t) clip8(sum3 >> out_shift);
              *(pOutBuffer2 + 1) = (uint8_t) clip8(sum4 >> out_shift);
            }
          }
        ...
        if(dim_out_y & 0x1){
          int32_t a_rollback_r = 4 - im2col_size;
          int32_t w_rollback_r = 4 - kernel_size;
          ...
          do{
            ...
            ptrA = MacLoadUpdate(ptrA);
            sum2 = MacLoad4(1, 0, 1, 1, ptrA, sum2);
            ptrA = MacLoadUpdate(ptrA);
            ptrB = MacLoadInit(0, 1, 0, 0, ptrB);
            j++;
          }while(j<colCnt);
