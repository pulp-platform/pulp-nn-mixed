/*
 * xpulp_nn_mix_depthwise_u2_i8_i8.c
...
void xpulp_nn_mix_depthwise_u2_i8_i8(
                        uint8_t *pIn,
                        ...
                        uint8_t flag_batch_norm)
{
  ...
  int32_t a_rollback = ((dim_out_y >> 1) * stride_y * dim_kernel_x) - (3 * a_stride);
  int32_t a_rollback2 = 4 - ((dim_out_y >> 1) * stride_y * dim_kernel_x) - (3 * a_stride);
  ...
  for(int i_ch = start_channel; i_ch < stop_channel; i_ch++)
  {
    ...
    if(padding_x_left > 0)
    {
      do
      {
        ...
        do
        {
          ...
          if(leftCnt)
          {
            ...
            do
            {
              ...
              uint8_t x_2 = *(uint8_t *) (pIm2Col - 1 + ((dim_out_y >> 1) * stride_y * dim_kernel_x));
              sum += x * w;
              sum5 += x_2 * w;
              int8_t w2 = *(int8_t *) pWt2++;
              uint8_t x2 = *(uint8_t *) pIm2Col2++;
              uint8_t x2_2 = *(uint8_t *) (pIm2Col2 - 1 + ((dim_out_y >> 1) * stride_y * dim_kernel_x));
              sum2 += x2 * w2;
              sum6 += x2_2 * w2;
              int8_t w3 = *(int8_t *) pWt3++;
              uint8_t x3 = *(uint8_t *) pIm2Col3++;
              uint8_t x3_2 = *(uint8_t *) (pIm2Col3 - 1 + ((dim_out_y >> 1) * stride_y * dim_kernel_x));
              sum3 += x3 * w3;
              sum7 += x3_2 * w3;
              int8_t w4 = *(int8_t *) pWt4++;
              uint8_t x4 = *(uint8_t *) pIm2Col4++;
              uint8_t x4_2 = *(uint8_t *) (pIm2Col4 - 1 + ((dim_out_y >> 1) * stride_y * dim_kernel_x));
              sum4 += x4 * w4;
              sum8 += x4_2 * w4;
              j++;
            }while(j<leftCnt);
        ...
        if(dim_out_y & 0x1){
          int32_t a_rollback_r = 4 - (3*im2col_size);
          int32_t w_rollback_r = 4 - (3*kernel_size);
          A_ROLLBACK(a_rollback_r);
          W_ROLLBACK(w_rollback_r);
          pWt = pWeight + i_wt_ch;
          pWt2 = pWt + kernel_size;
          pWt3 = pWt2 + kernel_size;
          pWt4 = pWt3 + kernel_size;
          ...
          do{
            ...
            ptrA = MacLoadInit(1, 0, 1, 0, ptrA);
            ptrA = MacLoadInit(1, 0, 2, 0, ptrA);
            sum4 = MacLoad4(1, 0, 3, 1, ptrA, sum4);
            ptrA = MacLoadUpdate(ptrA);
            j++;
          }while(j<colCnt);
          if(leftCnt){
          ...
          if (flag_batch_norm && flag_relu)
          {
            *pOutBuffer2 = pulp_nn_bn_quant_i8(sum, *k1, *lambda1, out_shift);
            *(pOutBuffer2 + 1) = pulp_nn_bn_quant_i8(sum2, *(k1 + 1), *(lambda1 + 1), out_shift);
            *(pOutBuffer2 + 2) = pulp_nn_bn_quant_i8(sum3, *(k1 + 2), *(lambda1 + 2), out_shift);
            *(pOutBuffer2 + 3) = pulp_nn_bn_quant_i8(sum4, *(k1 + 3), *(lambda1 + 3), out_shift);
          }
          else
          {
            if(flag_relu == 1)
            {
              *pOutBuffer2 = pulp_nn_quant_i8(sum, out_mult, out_shift);
              *(pOutBuffer2 + 1) = pulp_nn_quant_i8(sum2, out_mult, out_shift);
              *(pOutBuffer2 + 2) = pulp_nn_quant_i8(sum3, out_mult, out_shift);
              *(pOutBuffer2 + 3) = pulp_nn_quant_i8(sum4, out_mult, out_shift);
            }
            else
            {
              *pOutBuffer2 = (int8_t) clips8(sum >> out_shift);
              *(pOutBuffer2 + 1) = (int8_t) clips8(sum2 >> out_shift);
              *(pOutBuffer2 + 2) = (int8_t) clips8(sum3 >> out_shift);
              *(pOutBuffer2 + 3) = (int8_t) clips8(sum4 >> out_shift);
            }
          }
          A_ROLLBACK(a_rollback);
          W_ROLLBACK(w_rollback);
    ...
    do
    {
      ...
      if(dim_out_y & 0x1){
          A_STRIDE(im2col_size);
          W_STRIDE(kernel_size);
