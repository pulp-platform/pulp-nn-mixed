              ...
              sum[1] = 0;
                ...
                if (flag_requant) {
                  out_large = (sum[0] * lambda + out_add) >> out_shift;
                    out_el |= (clips22(out_large) << (in_iter_cnt * 4 + 0));
                  out_large = (sum[1] * lambda + out_add) >> out_shift;
                    out_el |= (clips22(out_large) << (in_iter_cnt * 4 + 2));
                  } else {
                  out_large = sum[0] / kernel_size_tot;
                  out_el |= (clips2(out_large) << (in_iter_cnt * 4 + 0));
                  out_large = sum[1] / kernel_size_tot;
                  out_el |= (clips2(out_large) << (in_iter_cnt * 4 + 2));
                }
                if (in_iter_cnt++ == 1) {
