...
uint8_t * __attribute__((noinline)) xpulp_nn_matmul_u1_u8_i1(
                        uint8_t *pIn,
                        int8_t *pBias,
                        uint8_t *pOut,
                        uint8_t *pOut2,
                        ...
                        uint8_t flag_batch_norm)
{
  ...
  uint16_t num_col_im2col_a = PACK_INT1_SIZE(num_col_im2col);
  ...
  for(int i=0; i < (ch_out >> 2); i++)
  {
    ...
    for(int j=0; j<(num_col_im2col >> 5); j++)
    {
      ...
      sum  = MacLoad32(0, 0, 0, 0, ptrA, sum);
      sum2 = MacLoad32(0, 0, 1, 0, ptrA2, sum2);
      sum3 = MacLoad32(0, 0, 2, 0, ptrA3, sum3);
      sum4 = MacLoad32(0, 1, 3, 0, ptrB, sum4);
      ptrB = MacLoadUpdate(ptrB);

      sum5 = MacLoad32(1, 0, 0, 1, ptrA, sum5);
      ptrA = MacLoadUpdate(ptrA);

      sum6 = MacLoad32(1, 0, 1, 1, ptrA2, sum6);
      ptrA2 = MacLoadUpdate(ptrA2);

      sum7 = MacLoad32(1, 0, 2, 1, ptrA3, sum7);
      ptrA3 = MacLoadUpdate(ptrA3);

      sum8 = MacLoad32(1, 0, 3, 1, ptrA4, sum8);
  ...
  while(chan_left)
  {
    ...
    for(int j=0; j < (num_col_im2col >> 5); j++)
    {
      ...
      sum  = MacLoad32(0, 1, 0, 0, ptrB, sum);
      ptrB = MacLoadUpdate(ptrB);

      sum2 = MacLoad32(1, 0, 0, 1, ptrA, sum2);
      ptrA = MacLoadUpdate(ptrA);
    }
    int col_cnt_im2col = num_col_im2col & 0x1f;
