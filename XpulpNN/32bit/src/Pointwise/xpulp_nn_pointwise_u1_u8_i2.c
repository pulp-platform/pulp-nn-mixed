...
void __attribute__((noinline)) xpulp_nn_pointwise_u1_u8_i2(
                        uint8_t *pIn,
                        ...
                        uint8_t flag_batch_norm)
{
  ...
  uint8_t *pIm2Col = pIm2ColBuffer + (2 * core_id * PACK_INT2_SIZE(ch_in));
  ...
  for (i_out_y = start_pixel; i_out_y < stop_pixel; i_out_y++)
  {
    ...
    {
      xpulp_nn_im2col_u1_to_u2(pIn + (i_out_x * ch_in_r) + (i_out_y * dim_in_x * ch_in_r), pIm2Col, ch_in<<1);
