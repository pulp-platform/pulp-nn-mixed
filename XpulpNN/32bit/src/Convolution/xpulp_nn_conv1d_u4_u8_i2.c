...
void __attribute__((noinline)) xpulp_nn_conv1d_u4_u8_i2(
                        uint8_t *pIn,
                        ...
                        uint8_t flag_batch_norm)
{
  ...
  uint8_t * pIm2ColBase = pIm2ColBuffer + (2 * core_id * PACK_INT4_SIZE(ch_in) * dim_kernel_x);
  int i_out_x, i_ker_x;
  int Log2Core;
  ...
  for (i_out_x = start_pixel; i_out_x < stop_pixel; i_out_x++)
  {
      ...
      if(pIm2Col == (pIm2ColBase + ((PACK_INT4_SIZE(ch_in) * dim_kernel_x) << 1)))
      {
