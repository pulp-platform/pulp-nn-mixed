...
void __attribute__((noinline)) xpulp_nn_conv_u1_u2_i1(
                        uint8_t *pIn,
                        ...
                        uint8_t flag_batch_norm)
{
  ...
  uint8_t * pIm2ColBase = pIm2ColBuffer + (2 * core_id * PACK_INT1_SIZE(ch_in) * dim_kernel_x * dim_kernel_y);
  ...
  for (i_out_y = start_pixel; i_out_y < stop_pixel; i_out_y++)
  {
    for(i_out_x=(section * dim_out_x_r); i_out_x<(dim_out_x_r + (section * (dim_out_x_r + flag_dim_out_x_odd))); i_out_x++)
    {
      if(i_out_y < padding_y_top)
      {
        for(i_ker_y=((i_out_y * stride_y) - padding_y_top); i_ker_y<((i_out_y * stride_y) - padding_y_top + dim_kernel_y); i_ker_y++)
        {
          for(i_ker_x=((i_out_x * stride_x) - padding_x_left); i_ker_x<((i_out_x * stride_x) - padding_x_left + dim_kernel_x); i_ker_x++)
          {
            ...
            {
              xpulp_nn_zero_mem_u1(pIm2Col, ch_in);
            }
            else
            {
              xpulp_nn_im2col_u1_to_u1((uint8_t*) (pIn + ((i_ker_y * dim_in_x + i_ker_x) * ch_in_r)), pIm2Col, ch_in);
            }
            pIm2Col+=PACK_INT1_SIZE(ch_in);
          }
        }
      }
      else if(i_out_y < dim_out_y - padding_y_bottom)
      {
        ...
        else if(i_out_x < (dim_out_x - padding_x_right))
        {
          for(i_ker_y=((i_out_y * stride_y) - padding_y_top); i_ker_y<((i_out_y * stride_y) - padding_y_top + dim_kernel_y); i_ker_y++)
          {
            xpulp_nn_im2col_u1_to_u1((uint8_t*) pIn + (i_ker_y * dim_in_x + i_out_x * stride_x - padding_x_left)*ch_in_r,pIm2Col,ch_in * dim_kernel_x);
            pIm2Col+=PACK_INT1_SIZE(ch_in * dim_kernel_x);
          }
        }
        else
        {
      ...
      if(pIm2Col == (pIm2ColBase + ((PACK_INT1_SIZE(ch_in) * dim_kernel_x * dim_kernel_y) << 1)))
      {
        pOutBuffer = xpulp_nn_matmul_u1_u2_i1(
          pIm2ColBase,
          pBias,
          pOutBuffer,
          pOutBuffer + ch_out_r,
    ...
    if(pIm2Col != pIm2ColBase)
    {
      ...
      uint16_t num_col_im2col_w = PACK_INT1_SIZE(ch_in) * dim_kernel_x * dim_kernel_y;
      ...
      for(i = 0; i < ch_out; i++)
      {
        ...
        for(int j=0; j < (num_col_im2col >> 5); j++)
        {
        }

        int col_cnt_im2col = num_col_im2col & 0x1f;

        if(col_cnt_im2col)
        {
          uint16_t loop_cnt_im2col_w = (num_col_im2col >> 5) << 2;
          pA+=loop_cnt_im2col_w;

          uint16_t loop_cnt_im2col_a = (num_col_im2col >> 5) << 2;
