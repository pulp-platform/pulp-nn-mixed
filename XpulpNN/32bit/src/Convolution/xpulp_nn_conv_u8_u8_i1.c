...
void __attribute__((noinline)) xpulp_nn_conv_u8_u8_i1(
                        uint8_t *pIn,
                        ...
                        uint8_t flag_batch_norm)
{
  ...
  for (i_out_y = start_pixel; i_out_y < stop_pixel; i_out_y++)
  {
    for(i_out_x=(section * dim_out_x_r); i_out_x<(dim_out_x_r + (section * (dim_out_x_r + flag_dim_out_x_odd))); i_out_x++)
    {
      ...
      {
        pOutBuffer = xpulp_nn_matmul_u8_u8_i1(
          pIm2ColBase,
          pBias,
          pOutBuffer,
          pOutBuffer + ch_out_r,
    ...
    if(pIm2Col != pIm2ColBase)
    {
      ...
      for(i = 0; i < ch_out; i++)
      {
        ...
        if(col_cnt_im2col)
        {
          ...
          uint16_t loop_cnt_im2col_a = (num_col_im2col >> 5) << 5;
