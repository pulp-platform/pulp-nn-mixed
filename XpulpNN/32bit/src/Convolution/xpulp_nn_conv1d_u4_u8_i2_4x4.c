/*
 * xpulp_nn_conv1d_u4_u8_i2_4x4.c
...
void __attribute__((noinline)) xpulp_nn_conv1d_u4_u8_i2_4x4(
                        uint8_t *pIn,
                        ...
                        uint8_t flag_batch_norm)
{
  ...
  uint8_t * pIm2ColBase = pIm2ColBuffer + (4 * core_id * PACK_INT4_SIZE(ch_in) * dim_kernel_x);
  int i_out_x, i_ker_x;
  int Log2Core;
  ...
  uint8_t *pOutBuffer = pOut + (start_pixel * ch_out_r) + (section * ch_out_r);
  ...
  for (i_out_x = start_pixel; i_out_x < stop_pixel; i_out_x++)
  {
      if(i_out_x < padding_x_left)
      {
        for(i_ker_x=i_out_x * stride_x - padding_x_left; i_ker_x < i_out_x * stride_x - padding_x_left + dim_kernel_x * (1 + dil) - dil; i_ker_x += (1 + dil))
        {
            ...
            {
              xpulp_nn_im2col_u4_to_u4((uint8_t*) pIn + i_ker_x * ch_in_r, pIm2Col, ch_in);
            }
      ...
      if(pIm2Col == (pIm2ColBase + ((PACK_INT4_SIZE(ch_in) * dim_kernel_x) << 2)))
      {
    ...
    if(pIm2Col != pIm2ColBase)
    {
      ...
      uint16_t num_col_im2col_w = PACK_INT2_SIZE(ch_in) * dim_kernel_x;
      ...
      for(i = 0; i < ch_out; i++)
      {
        ...
        if(col_cnt_im2col)
        {
          ...
          do
          {
            ...
            uint8_t inB1 = (uint8_t) bitextu((uint8_t) *pB, 4, 0);
            sum += inA1 * inB1;
