...
uint8_t *xpulp_nn_matmul_u8_u8_i4(
  ...
  for(int i=0; i < (ch_out >> 2); i++)
  {
    ...
    for(int j=0; j<(num_col_im2col >> log2(8)); j++)
    {
...
//       uint8_t i_o = i & 0x01;
//       out[i_o] = pulp_nn_bn_quant_u8(sum, *k, *lambda, out_shift);
//       out2[i_o] = pulp_nn_bn_quant_u8(sum2, *k, *lambda, out_shift);
//       k++;
//       lambda++;
//       if(i_o == 0x01)
//       {
...
// %elif config.kernel.out_data_t == 4:
//         uint8_t i_o = i & 0x01;
//         out[i_o] = pulp_nn_quant_u8(sum, out_mult, out_shift);
//         out2[i_o] = pulp_nn_quant_u8(sum2, out_mult, out_shift);
//         if(i_o == 0x01)
//         {
//           *pOut = bitins(out[0], n_mask, out[1], mask, off);
//           *pOut2 = bitins(out2[0], n_mask, out2[1], mask, off);
