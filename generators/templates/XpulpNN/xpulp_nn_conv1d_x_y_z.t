...
%if config.kernel.matmul_fmt == '4x2':
  ${pt_in} * pIm2ColBase = pIm2ColBuffer + (2 * core_id * PACK_INT${config.max_precision}_SIZE(ch_in) * dim_kernel_x);
%elif config.kernel.matmul_fmt == '4x4':
  ${pt_in} * pIm2ColBase = pIm2ColBuffer + (4 * core_id * PACK_INT${config.max_precision}_SIZE(ch_in) * dim_kernel_x);
%endif
  int i_out_x, i_ker_x;
  int Log2Core;
  ...
  ${pt_out} *pOutBuffer = pOut + (start_pixel * ch_out_r) + (section * ch_out_r);
  ...
  for (i_out_x = start_pixel; i_out_x < stop_pixel; i_out_x++)
  {
      if(i_out_x < padding_x_left)
      {
        for(i_ker_x=i_out_x * stride_x - padding_x_left; i_ker_x < i_out_x * stride_x - padding_x_left + dim_kernel_x * (1 + dil) - dil; i_ker_x += (1 + dil))
        {
            ...
            {
              ${config.im2col_fn}((uint8_t*) pIn + i_ker_x * ch_in_r, pIm2Col, ch_in);
            }
...
%if config.kernel.matmul_fmt == '4x2':
      if(pIm2Col == (pIm2ColBase + ((PACK_INT${config.max_precision}_SIZE(ch_in) * dim_kernel_x) << 1)))
%elif config.kernel.matmul_fmt == '4x4':
      if(pIm2Col == (pIm2ColBase + ((PACK_INT${config.max_precision}_SIZE(ch_in) * dim_kernel_x) << 2)))
%endif
      {
...
%endif
      ...
      uint16_t num_col_im2col_w = PACK_INT${config.kernel.wt_data_t}_SIZE(ch_in) * dim_kernel_x;
...
%if config.kernel.wt_data_t < config.kernel.in_data_t:
            int8_t inA1 = (int8_t) bitext((int) *pA, 2, 0);
            ${pt_in} inB1 = (${pt_in}) ${bex}((${pt_in}) *pB, 4, 0);
            sum += inA1 * inB1;
            inA1 = (int8_t) bitext((int) *pA, 2, 2);
