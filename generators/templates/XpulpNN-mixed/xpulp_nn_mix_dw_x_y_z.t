...
%if config.less_precision == 8:
  int32_t a_stride = (dim_out_y >> 1) * stride_y * dim_kernel_x;
  int32_t w_stride = 0;
  int32_t a_rollback = 4 - a_stride;
...
%endif
  ...
  %elif config.less_precision == 4:
            ...
            sum2 = ${macload_fn}(0, 1, 1, 1, ptrB, sum2);
            ptrB = MacLoadUpdate(ptrB);
            sum3 = ${macload_fn}(1, 0, 0, 0, ptrA, sum3);
            ptrA = MacLoadUpdate(ptrA);
            sum4 = ${macload_fn}(1, 0, 1, 1, ptrA, sum4);
            ptrA = MacLoadUpdate(ptrA);
            ptrB = MacLoadInit(0, 1, 0, 0, ptrB);
            A_ROLLBACK(a_rollback);
  %elif config.less_precision == 2:
            ...
            sum2 = ${macload_fn}(0, 1, 1, 1, ptrB, sum2);
            ptrB = MacLoadUpdate(ptrB);
            sum3 = ${macload_fn}(0, 1, 2, 0, ptrB, sum3);
            ptrB = MacLoadUpdate(ptrB);
            sum4 = ${macload_fn}(0, 1, 3, 1, ptrB, sum4);
            ptrB = MacLoadUpdate(ptrB);
            sum5 = ${macload_fn}(0, 1, 0, 0, ptrB, sum5);
            ptrB = MacLoadUpdate(ptrB);
            A_ROLLBACK(a_rollback2);
            sum6 = ${macload_fn}(0, 1, 1, 1, ptrB, sum6);
            ptrB = MacLoadUpdate(ptrB);
            ...
            ptrA = MacLoadInit(1, 0, 1, 0, ptrA);
            ptrA = MacLoadInit(1, 0, 2, 0, ptrA);
            sum8 = ${macload_fn}(1, 0, 3, 1, ptrA, sum8);
            ptrA = MacLoadUpdate(ptrA);
            A_ROLLBACK(a_rollback);
  %endif
            j++;
...
%if config.less_precision == 8:
  ...
  %elif config.less_precision == 4:
            ...
            sum  = ${macload_fn}(1, 0, 0, 0, ptrA, sum);
            ptrA = MacLoadUpdate(ptrA);
            sum2 = ${macload_fn}(1, 0, 1, 1, ptrA, sum2);
            ptrA = MacLoadUpdate(ptrA);
            ptrB = MacLoadInit(0, 1, 0, 0, ptrB);
  %elif config.less_precision == 2:
            ptrB = MacLoadInit(0, 1, 0, 1, ptrB);
            ...
            ptrA = MacLoadInit(1, 0, 1, 0, ptrA);
            ptrA = MacLoadInit(1, 0, 2, 0, ptrA);
            sum4 = ${macload_fn}(1, 0, 3, 1, ptrA, sum4);
            ptrA = MacLoadUpdate(ptrA);
  %endif
            j++;
          }while(j<colCnt);
...
%endif
  ...
  %endif
          ...
          A_ROLLBACK(a_rollback);
          W_ROLLBACK(w_rollback);
%if (config.less_precision == 8) and (config.kernel.out_data_t == 8):
          A_SKIP("1");
%endif
        }
      i_out_x++;
